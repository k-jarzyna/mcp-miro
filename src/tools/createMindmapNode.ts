...
const createMindmapNodeTool: ToolSchema = {
  ...
  args: {
    ...
    data: z.object({
      ...
      parentId: z.string().optional().nullable().describe("ID of the parent node (if this is a child node)"),
      style: z.object({
        fillColor: z.string().optional().nullable().describe("Fill color for the node"),
        textColor: z.string().optional().nullable().describe("Text color for the node")
      }).optional().nullable()
