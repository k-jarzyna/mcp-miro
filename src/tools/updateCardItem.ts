...
const updateCardItemTool: ToolSchema = {
  ...
  args: {
    ...
    data: z.object({
      title: z.string().optional().nullable().describe("Updated title of the card"),
      description: z.string().optional().nullable().describe("Updated description of the card"),
      assigneeId: z.string().optional().nullable().describe("Updated user ID of the assignee"),
      dueDate: z.string().optional().nullable().describe("Updated due date for the card (ISO 8601 format)")
    }).optional().nullable().describe("The updated content and configuration of the card"),
    position: z.object({
      x: z.number().describe("Updated X coordinate of the card"),
      y: z.number().describe("Updated Y coordinate of the card")
    }).optional().nullable().describe("Updated position of the card on the board"),
    geometry: z.object({
      width: z.number().optional().nullable().describe("Updated width of the card"),
      height: z.number().optional().nullable().describe("Updated height of the card"),
      rotation: z.number().optional().nullable().describe("Updated rotation angle of the card")
    }).optional().nullable().describe("Updated dimensions of the card"),
    style: z.object({
      cardTheme: z.string().optional().nullable().describe("Updated color of the card")
    }).optional().nullable().describe("Updated style configuration of the card")
  },
