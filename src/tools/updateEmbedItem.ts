...
const updateEmbedItemTool: ToolSchema = {
  ...
  args: {
    ...
    data: z.object({
      mode: z.string().optional().nullable().describe("Updated mode of the embed (normal, inline, etc.)")
    }).optional().nullable().describe("The updated configuration of the embed"),
    position: z.object({
      x: z.number().describe("Updated X coordinate of the embed"),
      y: z.number().describe("Updated Y coordinate of the embed"),
      origin: z.string().optional().nullable().describe("Origin of the embed (center, top-left, etc.)"),
      relativeTo: z.string().optional().nullable().describe("Reference point (canvas_center, etc.)")
    }).optional().nullable().describe("Updated position of the embed on the board"),
    geometry: z.object({
      width: z.number().optional().nullable().describe("Updated width of the embed"),
      height: z.number().optional().nullable().describe("Updated height of the embed")
    })
    .optional().nullable()
