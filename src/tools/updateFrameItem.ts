...
const updateFrameItemTool: ToolSchema = {
  ...
  args: {
    ...
    data: z.object({
      title: z.string().optional().nullable().describe("Title of the frame. This title appears at the top of the frame."),
      format: z.string().optional().nullable().describe("Format of the frame. Only 'custom' is supported currently."),
      type: z.string().optional().nullable().describe("Type of the frame. Only 'freeform' is supported currently."),
      showContent: z.boolean().optional().nullable().describe("Hide or reveal the content inside a frame (Enterprise plan only).")
    }).optional().nullable().describe("The updated content and configuration of the frame"),
    position: z.object({
      x: z.number().describe("X coordinate of the frame"),
      y: z.number().describe("Y coordinate of the frame")
    }).optional().nullable().describe("Updated position of the frame on the board"),
    geometry: z.object({
      width: z.number().optional().nullable().describe("Width of the frame"),
      height: z.number().optional().nullable().describe("Height of the frame")
    }).optional().nullable().describe("Updated dimensions of the frame"),
    style: z.object({
      fillColor: z.string().optional().nullable().describe("Fill color for the frame. Hex values like #f5f6f8, #d5f692, etc.")
    }).optional().nullable().describe("Updated style configuration of the frame")
  },
